...
namespace BrTypes.Tests
{
    public class Cpf2Tests
    {
        ...
        public void Deve_falhar_ao_instanciar_cpf_com_digito_errado()
        {
            ...
            Action act = () => Cpf2.Parse(cpfInvalido);
        ...
        {
            Action conversaoValida = () => { Cpf2 cpf1 = CpfUtils.GerarCpf(); };
            Action conversaoInvalida = () => { Cpf2 cpf1 = CpfUtils.GerarCpfComDigitosInvalidos(); };
            
            conversaoValida.Should().NotThrow<CpfInvalidoException>();
            conversaoInvalida.Should().ThrowExactly<CpfInvalidoException>();
        }

        // [Theory(Skip = "Does not implements Base property yet")]
        ...
        // {
        //     var cpf = Cpf2.Parse(cpfCompleto);
        //     cpf.Base.Should().Be(cpfBase);
        // }

        // [Theory(Skip = "Does not implements DV property yet")]
        ...
        public void Cpfs_com_mesmo_numero_devem_ser_considerados_iguais()
        {
            ...
            Assert.True(cpf1 == cpf2, "Cpfs são iguais");
        }
        ...
        [Fact(Skip = "Does not implements ToString with mask yet")]
        public void Deve_formatar_cpf_com_estilo_Padrao()
        {
            // var cpf = Cpf2.Parse("61709754079");
            // cpf2.ToString(EstiloFormatacaoCpf.Padrao).Should().Be("617.097.540-79");
        }
        
        [Fact(Skip = "Does not implements ToString with mask yet")]
        public void Deve_formatar_cpf_com_estilo_Nenhum()
        {
            // var cpf = Cpf2.Parse("617.097.540-79");
